...
class graph
{
	...
	graph(int n): n(n)
	{
		...
		for (int i = 0; i < n; i++) 
		{
			grid.push_back(vector<char>(n)); // Add an empty row
		}
	...
	int computerMove(int m)
	{
		...
		for(int j=0; j<n*n; ++j)
		{
			...
			else
			{
				...
				while(trials-->0)
				{
					cout<<"\nExecuting trial no. "<<100-trials;
					//store vtemp in g.data
					for(int i=0; i<(n*n); ++i)
					{
						g[i].data=vtemp.at(i);
					}
					...
					if((n*n/2)%2!=0) v.at(n*n/2)='X';//first player character
					else
					{
					...
					random_shuffle(v.begin()+m, v.end()-(m-1));
					int k=0;
					for(auto i=v.begin()+m; i!=(v.end()-(m-1)); ++i)
					{
						while(g[k].data!=0)	k++;
						g[k].data=*i;
					}
		...
		for(auto i : g)
		{
			gtemp.push_back(i.data);
		}
		while(trials-->0)
		{
			vector<int> g2(gtemp);
			std::shuffle(v.begin()+m, v.end()-(m-1), engine);
			auto j=v.begin()+m;
...
int main(int argc, char **argv)
{
	...
	*/
    ...
    std::cout<<"\nDone in "<< time(NULL)-t0 <<"seconds.\nx="<<x<<" y="<<y<<" ycalc="<<t2-x;
    int max=0, pos=0;
    cout<<endl;
    for(int i=0, k=count.size(); i<k; ++i)
	{
		cout<<i<<":"<<count.at(i)<<" ";
		if(count.at(i)>max)
		{ 
	...
	board.display();
	cout<<"\nMax="<<max<<" at position row:"<<(pos/n)+1<<" col:"<<(pos%n)+1;
	cout<<"\nProbability="<<1.0*max/t2;
	*/
